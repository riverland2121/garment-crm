...
import { Plus, Search, Download, Upload, Edit, Trash2, Clock } from 'lucide-react'
...
export default function ServicesPage() {
  ...
  const serviceProviders = ['Tailor - Ramesh Kumar', 'Embroidery Studio']
  ...
  const handleExport = () => {
    const csvData = filteredServices.map(s => ({
      ...
      'GST Rate': s.gstRate,
      'Estimated Time (Days)': s.estimatedTime,
      Status: s.status
    }))
  ...
  const handleDownloadTemplate = () => {
    const template = `Name,Description,Unit,Provider,Cost Per Unit,HSN/SAC Code,GST Rate,Estimated Time (Days),Status
Stitching Service,Basic garment stitching,Piece,Tailor Shop,150,998321,Not Applicable,2,Active
Hand Embroidery,Traditional hand embroidery,Piece,Embroidery Studio,500,998322,Not Applicable,8,Active`
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="col-span-2">
                    ...
                    >
                      <option value="">Select or type new service</option>
                    ...
                    <input
                      type="text"
                      placeholder="Or type new service name"
                      value={formData.name}
                      onChange={(e) => setFormData({...formData, name: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 mt-2"
                    />
                  </div>
